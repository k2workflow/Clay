...
{
    internal class SwitchBuilder<K, V> : IDynamicSwitch<K, V>
    {
        private readonly Func<K, int> _indexer;
        private readonly IReadOnlyList<V> _values;

        public SwitchBuilder(IReadOnlyDictionary<K, V> cases)
        {
        ...
        private Func<K, int> Build(IReadOnlyDictionary<K, int> cases)
        {
            ...
            var mi = typeof(SwitchBuilder<K, V>).GetMethod(nameof(Normalize), BindingFlags.Instance | BindingFlags.NonPublic);
