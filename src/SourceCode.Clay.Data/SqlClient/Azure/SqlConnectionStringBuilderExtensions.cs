...
namespace SourceCode.Clay.Data.SqlClient.Azure
{
    ...
    public static class SqlConnectionStringBuilderAzureExtensions
    {
        ...
        public static SqlConnectionStringBuilder MakeRobust(this SqlConnectionStringBuilder sqlCsb, SqlConnectionRetryOptions options)
        {
            ...
            {
                // Datasource len p1 p2 p1>=1 p2<len ok .database.b 11 0 10 F T F a.database. 11 1 11
                // T F F a.database.b 12 1 11 T T T a.database.windows.net 22 1 11 T T T
                ...
                if (p1 >= 1 && p2 < sqlCsb.DataSource.Length)
                {
                    ...
                    {
                        // OK, it seems this is an AzureDb connection string Per MSFT security
                        // guidelines, set the following values
                        sqlCsb.TrustServerCertificate = false;
                        sqlCsb.Encrypt = true;

                        // AzureDb only supports TCP:1433, though SqlClient wastes time trying to
                        // negotiate named pipes, etc
